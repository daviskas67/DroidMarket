...
public class ThemeManager {
    ...
    public static void apply(Activity activity) {
        ...
        String theme = prefs.getString(KEY_THEME, THEME_MARKET);
        if (THEME_HOLO.equals(theme)) {
            activity.setTheme(R.style.Theme_DroidMarket_Holo);
        } else if (THEME_MATERIAL.equals(theme)) {
    ...
    public static String getCurrent(Activity activity) {
        SharedPreferences prefs = activity.getSharedPreferences(PREFS_NAME, Activity.MODE_PRIVATE);
        return prefs.getString(KEY_THEME, THEME_MARKET);
    }
    ...
    public static String nextTheme(String current) {
        if (THEME_MARKET.equals(current)) return THEME_HOLO;
        if (THEME_HOLO.equals(current)) return THEME_MATERIAL;
        if (THEME_MATERIAL.equals(current)) return THEME_AMOLED;
        if (THEME_AMOLED.equals(current)) return THEME_MARKET;
        return THEME_MARKET;
    }
}
